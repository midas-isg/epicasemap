...
L.HeatLayer = (L.Layer?L.Layer : L.Class).extend({
	...
	_initCanvas : function() {
		...
		a = this._map.options.zoomAnimation&&L.Browser.any3d;
	...
	_redraw : function() {
		...
		for(t = 0, i = this._latlngs.length; i>t; t++)
			if(m.contains(this._latlngs[t])) {
				...
				y = void 0!== this._latlngs[t].alt?this._latlngs[t].alt : void 0!== this._latlngs[t][2]?+this._latlngs[t][2] : 1;
		...
		for(t = 0, i = f.length; i>t; t++)
			if(f[t])
				for(h = 0, o = f[t].length;o>h; h++)
					s = f[t][h], s&&_.push([Math.round(s[0]), Math.round(s[1]), Math.min(s[2], 1)]);
